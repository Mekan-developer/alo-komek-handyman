...
### Store-review accounts (fixed OTP)

App Store and Google Play reviewers have no Turkmen SIM, so a whitelisted phone can sign in with a fixed code instead of a real SMS:

```dotenv
OTP_TEST_PHONES=+99362222222   # comma-separated; empty disables the bypass
OTP_TEST_CODE=010101
```

`OtpTestAccountService` owns the check (digits-only comparison, so `+99362222222` and `99362222222` both match). For these phones `DispatchOtpAction` returns `delivery: "sms"` immediately — nothing is generated, sent to the gateway, cached or parked in `pending_otps` — and both `VerifyMasterOtpAction` / `VerifyClientOtpAction` accept `OTP_TEST_CODE` in place of the cached code.

Everything else still applies: a master on this phone must exist in `masters` and pass `is_active` + `hasActiveAccess()` (`MasterSeeder` creates a `Demo Master` for the first configured phone), and a client is auto-registered on first verify as usual.

> **Both variables must be set for the bypass to exist** — the config defaults are empty, so a deploy that never sets them has no fixed-code login at all. Clear `OTP_TEST_PHONES` to revoke it.
