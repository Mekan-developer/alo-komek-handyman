...
- Tasks without a price are ignored. When no task is priced, `final_price` is reset to `null`,
  so the "completed without a price" warnings and `CreditMasterBalanceAction` keep behaving as
  before.
