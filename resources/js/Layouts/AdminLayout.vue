...
/**
 * Realtime "you weren't looking" counters — bumped when an order/client event
 * arrives while the user is on another section, cleared once they visit it.
 * Deliberately in-memory only (resets on reload), unlike the DB-backed
 * `badgeKey` counters below.
 */
const realtimeBadges = ref({ orders: 0, clients: 0 })

/** Badge counters live in shared Inertia props, so they stay fresh on every visit. */
function badgeCount(item) {
    if (item.badgeKey) {
        return page.props[item.badgeKey] ?? 0
    }
    if (item.realtimeBadgeKey) {
        return realtimeBadges.value[item.realtimeBadgeKey] ?? 0
    }
    return 0
}
...
/** Only counts while the user is elsewhere — visiting the section clears it via the watcher below. */
function bumpRealtimeBadge(key, routeName) {
    if (isCurrentRoute(routeName)) { return }
    realtimeBadges.value[key]++
}

watch(() => page.url, () => {
    if (isCurrentRoute('orders.index')) { realtimeBadges.value.orders = 0 }
    if (isCurrentRoute('clients.index')) { realtimeBadges.value.clients = 0 }
})
...
function handleNewOrder(payload) {
    ...
    bumpRealtimeBadge('orders', 'orders.index')
    refreshStoredNotifications()
}

function handleNewClient(payload) {
    notificationStore.info(t('clients.notifications.new_client', {
        phone: payload.phone ?? '—',
    }))
    playAlarmSound()

    bumpRealtimeBadge('clients', 'clients.index')
}
    ...
    <div class="flex h-screen overflow-hidden bg-gray-100 dark:bg-slate-900">
        ...
        >
            ...
            <nav class="flex-1 space-y-6 overflow-y-auto px-3 py-4">
                <div v-for="group in mainNavGroups" :key="group.key" class="space-y-1">
                    <p class="px-3 pb-1 text-[11px] font-semibold uppercase tracking-wider text-slate-500">
                        {{ t(group.titleKey) }}
                    </p>
                    <Link
            ...
            <div
                ...
                :key="group.key"
                class="shrink-0 space-y-1 border-t border-slate-700 px-3 py-3"
            >
                <p class="px-3 pb-1 text-[11px] font-semibold uppercase tracking-wider text-slate-500">
                    {{ t(group.titleKey) }}
                </p>
